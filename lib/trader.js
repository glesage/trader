...
module.exports = function (fees, placeOrder)
{
    ...
    const average = new Average(resolution, minTrades);
    ...
    {
        const tic = average.updatedAverage(trade);
        ...
        if (data.positions && data.positions.length > 0)
        {
            ...
            if (tic < self.stopLossPrice)
            {
                ...
                return placeOrder(sellOrder(btfnxPrice(tic), position.amount));
            }
            ...
            {
                console.log('ticket dropped below best price');
                console.log('best price: ' + self.activeData.sellPrice);
                console.log('current: ' + tic);
