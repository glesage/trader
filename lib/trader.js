...
module.exports = function (fees, placeOrder)
{
    ...
    function gotCurrentTicker(ticker, data)
    {
        ...
        if (data.balanceUSD > 0 && data.balanceUSD > (minTradeBTC * ticker))
        {
            ...
            var newSupportZone = getSupportZone(ticker);
            ...
            if (!self.activeData.buyPrice || !self.activeData.buyTime) return setPossibleBuy(ticker);

            // If the ticker has dropped since last time, just record this moment
            if (comparePrices(ticker, self.activeData.buyPrice) === -1) return setPossibleBuy(ticker);

            // If the ticker has increased above the lowest price recorded, buy
            if (ticker < getMaxIncrease(self.activeData.buyPrice))
            {
                var order = buyOrder(btfnxPrice(ticker), data.balanceUSD);
        ...
        else if (data.balanceBTC > 0 && data.balanceBTC > minTradeBTC)
        {
            ...
            if (ticker < self.activeData.stopLossZone)
            {
                var order = sellOrder(btfnxPrice(ticker), data.balanceBTC);
                return placeOrder(order);
            }
            ...
            if (!self.activeData.sellPrice || !self.activeData.sellTime) return setPossibleSell(ticker);

            // If the ticker has risen since last time, just record this moment
            if (comparePrices(ticker, self.activeData.sellPrice) === 1) return setPossibleSell(ticker);

            // If the ticker has dropped below the highest price recorded, sell
            if (ticker < getMaxDrop(self.activeData.sellPrice))
            {
                var order = sellOrder(btfnxPrice(ticker), data.balanceBTC);
                return placeOrder(order);
            }
